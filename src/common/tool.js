...
let search = function(text,keyword) {
  if(keyword==''||keyword==undefined)return text;
  if(text==''||text==undefined)return false;
  //console.log(text,keyword)
  //对搜索的文字进行特殊化
  var regex_str = function (str, keyword, insensitive){
    var regex = new RegExp('(<[^>]*>)|(\\b'+ escapeRegExp(keyword) +')', insensitive ? 'ig' : 'g');
  ...
  if((patt1.test(text))) {
    var rEnd = regex_str(text,keyword,true);
    return rEnd
  }else{
    return false
  }
